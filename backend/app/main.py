...
async def get_current_user(
    ...
    user = await crud.get_user_by_id(session, payload["user_id"])
...
async def register(user_in: schemas.UserCreate,
    ...
    user = await crud.create_user(
        session, user_in.username, user_in.email, user_in.password
    )
...
async def login(form_data: schemas.LoginInput,
                session: AsyncSession = Depends(get_session)):
...
async def list_notes(
    ...
    if q:
        return await crud.search_notes_by_title(session, current_user.id, q)

    return await crud.get_notes_by_owner(session, current_user.id)
...
async def update_note(
    ...
    updated = await crud.update_note(
        session, note, note_in.title, note_in.content
    )
